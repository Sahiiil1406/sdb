...
ExecuteResult execute_statement(Statement* statement, Table* table) {
  ...
  return EXECUTE_SUCCESS; // Default return to avoid compiler warning
}
...
MetaCommandResult metadoCommand(InputBuffer* input_buffer,Table *table){
...
int main(int argc, char* argv[]) {
  if(argc<2){
    printf("Supply db name: ./a.out db_name\n");
    exit(EXIT_FAILURE);
  }
  ...
  while (true) {
    ...
    if (input_buffer->buffer[0] == '.') {
      switch (metadoCommand(input_buffer,table)) {
    ...
    switch (prepare_statement(input_buffer, &statement)) {
      ...
      case (PREPARE_UNRECOGNIZED_STATEMENT):
        printf("Unrecognized sdb keyword '%s'.\n", input_buffer->buffer);
        continue;
    }
